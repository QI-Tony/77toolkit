...
function createPrimaryHeader() {
  return `<header class="site-header">
      <nav class="nav" aria-label="Primary navigation">
        ...
        <div class="nav-links"><a href="/#tools">All tools</a><a href="/developer/">Developer</a><a href="/text/">Text</a><a href="/image/">Image</a><a href="/web/">Web</a><a href="/guides/">Guides</a></div>
      </nav>
    </header>`;
}
...
function createAdvertisingLoaders() {
  return `<script src="https://pl31227992.profitableratecpmnetwork.com/73/ef/51/73ef51cc952b9357951824c05b8a5c29.js"></script>
    <script src="https://pl31227993.profitableratecpmnetwork.com/e8/0c/f4/e80cf4e5014c8492739d4b636100e786.js"></script>`;
}
...
<html lang="en">
  ...
  <body>
    ...
    ${createLegalFooter()}
    ${noindex ? "" : createAdvertisingLoaders()}
  </body>
</html>
`;
}
...
function createSiteMapPage() {
  const categorySections = Object.entries(categoryContent)
    .map(([categoryName, category]) => {
      const tools = toolCatalog
        .filter((tool) => tool.category === categoryName)
        .map((tool) => `<li><a href="/tools/${escapeHtml(tool.slug)}/">${escapeHtml(tool.name)}</a><span>${escapeHtml(tool.description)}</span></li>`)
        .join("");
      return `<section class="site-map-section"><h2><a href="/${escapeHtml(category.slug)}/">${escapeHtml(category.title)}</a></h2><ul>${tools}</ul></section>`;
    })
    .join("");
  const guides = guideCatalog
    .map((guide) => `<li><a href="/guides/${escapeHtml(guide.slug)}/">${escapeHtml(guide.title)}</a><span>${escapeHtml(guide.description)}</span></li>`)
    .join("");
  const information = informationLinks
    .filter(([, href]) => href !== "/site-map/")
  ...
  return createStaticPage({
    slug: "site-map",
    title: "Site Map",
    description: "Browse every 77 Toolkit category, browser tool, practical guide, and site information page from one human-readable index.",
    body: `<header class="static-hero"><p class="static-eyebrow">Human-readable index</p><h1>Everything published on 77 Toolkit</h1><p>Use this page to browse the complete collection without search or filters. The XML sitemap for search engines remains available at <a href="/sitemap.xml">/sitemap.xml</a>.</p></header><div class="site-map-grid">${categorySections}<section class="site-map-section site-map-section--wide"><h2><a href="/guides/">Practical Guides</a></h2><ul>${guides}</ul></section><section class="site-map-section"><h2>Site information</h2><ul>${information}</ul></section></div>`,
  });
}
...
function createTrustPages() {
  return {
    about: createStaticPage({
      ...
      body: `<header class="static-hero"><p class="static-eyebrow">About the project</p><h1>Small tools, clearly explained</h1><p>77 Toolkit is an independent collection of focused browser utilities for developer data, text, images, and web work.</p></header><div class="trust-grid"><section class="trust-card"><h2>Useful by design</h2><p>Each page handles one understandable task and explains its method, limitations, and expected result.</p></section><section class="trust-card"><h2>Local where possible</h2><p>Public tools process pasted text and selected files in the browser whenever modern Web APIs make that practical.</p></section><section class="trust-card"><h2>Open development</h2><p>The source and change history are available on GitHub so problems and improvements can be reviewed publicly.</p></section></div><div class="static-prose"><h2>Why the site exists</h2><p>Many everyday digital tasks are too small for a full application but too sensitive for an unknown upload service. 77 Toolkit keeps these tasks in simple, bookmarkable pages and describes what each operation can and cannot guarantee.</p><h2>How tools are reviewed</h2><p>New tools are tested with representative valid, invalid, and boundary inputs, clear errors, keyboard-accessible controls, and production build checks. Documentation is updated when behavior or browser limitations change. The complete process is described in the <a href="/editorial-policy/">Editorial and Testing Policy</a>.</p><h2>Language and audience</h2><p>English is the primary language for navigation, publisher content, metadata, and Guides so every indexed page has one clear language. A small number of interactive interfaces also offer an optional Chinese control, but the site does not publish mass-translated duplicate pages.</p><h2>Funding and independence</h2><p>The site uses third-party pop-under and social bar advertising to help cover hosting and maintenance. Advertising does not change tool output, but an advertising interaction may open an external page, tab, or window. Details are provided in the <a href="/privacy/">Privacy Policy</a>.</p><h2>Project owner and feedback</h2><p>77 Toolkit is maintained through the <a href="https://github.com/QI-Tony/77toolkit">QI-Tony/77toolkit GitHub repository</a>. Bug reports, corrections, and feature suggestions can be submitted through its public issue tracker.</p><p class="static-meta">Last reviewed: September 7, 2026</p></div>`,
    }),
    "editorial-policy": createStaticPage({
      slug: "editorial-policy",
      title: "Editorial and Testing Policy",
      description: "How 77 Toolkit chooses, tests, documents, sources, reviews, corrects, and updates browser tools and practical guides.",
      body: `<header class="static-hero"><p class="static-eyebrow">Editorial and testing policy</p><h1>How a tool earns a published page</h1><p>77 Toolkit publishes working utilities together with original explanations. This policy describes what is checked before a page is included in navigation and the sitemap.</p></header><div class="trust-grid"><section class="trust-card"><h2>Function first</h2><p>A page must perform the task it promises with reviewable input, output, errors, and browser behavior.</p></section><section class="trust-card"><h2>Specific guidance</h2><p>Examples, limitations, alternatives, privacy notes, and checklists are written for that tool rather than copied across the catalog.</p></section><section class="trust-card"><h2>Traceable sources</h2><p>Long-form Guides link to primary specifications, standards bodies, or official project documentation whenever those sources exist.</p></section></div><div class="static-prose"><h2>Choosing new topics</h2><p>A new tool should solve a bounded task that can be completed safely in a browser and explained without pretending to replace professional validation. The collection avoids fake generators, unavailable features, pages created only for keywords, and dozens of near-identical variations of the same conversion.</p><h2>Functional review</h2><p>Before publication, the maintainer checks representative successful input, empty input, malformed input, boundary values, copying or downloading, keyboard operation, responsive layout, and the claim that data remains local. Security-sensitive tools must state what they do not verify. Image transformations preserve an original preview and warn about re-encoding effects.</p><h2>Editorial review</h2><p>Every tool page includes a task-specific introduction, steps, realistic example, method, limitations, FAQs, scenarios, result checklist, privacy context, and a better alternative where appropriate. Guides are organized around a real decision or debugging workflow rather than a search phrase. Automation may assist code and drafting, but published behavior is tested and factual claims are checked against the linked primary references.</p><h2>Language policy</h2><p>Indexed publisher content is written in English as the site's primary language. Optional Chinese controls in selected applications are clearly user-initiated. The project does not create automatic translated copies merely to multiply page count; a future language edition should receive human review, its own navigation, and correct hreflang relationships.</p><h2>Corrections and updates</h2><p>Each Guide displays publication and update dates. Corrections can be reported through the <a href="/contact/">Contact page</a> or public issue tracker. Material changes to tool behavior, standards, browser support, or safety guidance should update both the page and its reviewed date. The public Git history preserves what changed.</p><h2>Advertising separation</h2><p>Publisher content and application controls are designed before advertising. Ads must not imitate tool cards, navigation, results, copy buttons, or downloads, and no wording encourages clicks. Third-party ad formats can open external pages, tabs, or windows; the noindex 404 page does not load advertising scripts.</p><p class="static-meta">Effective and last reviewed: September 7, 2026</p></div>`,
    }),
    accessibility: createStaticPage({
      slug: "accessibility",
      title: "Accessibility Statement",
      description: "77 Toolkit's accessibility goals, current browser support, testing approach, known limitations, and reporting process.",
      body: `<header class="static-hero"><p class="static-eyebrow">Accessibility statement</p><h1>Tools should remain usable without precise pointing</h1><p>77 Toolkit aims to support keyboard navigation, readable zoom, clear labels, visible focus, and understandable status messages across its public pages.</p></header><div class="trust-grid"><section class="trust-card"><h2>Keyboard access</h2><p>Primary navigation, fields, buttons, disclosure panels, and downloads are designed to follow a logical tab order.</p></section><section class="trust-card"><h2>Readable presentation</h2><p>Layouts reflow for narrow screens, text uses relative sizing where practical, and essential information is not communicated by color alone.</p></section><section class="trust-card"><h2>Programmatic meaning</h2><p>Forms use labels, status regions announce results, tables have headings, and page landmarks support assistive navigation.</p></section></div><div class="static-prose"><h2>Target and testing</h2><p>The project uses WCAG 2.2 Level AA as a practical target, not as an unsupported claim of complete conformance. New shared tools are reviewed with keyboard-only operation, browser zoom, narrow layouts, visible focus, labeled controls, error messages, and contrast checks. Production build validation also catches broken internal navigation and missing page metadata.</p><h2>Known limitations</h2><p>The Color Spectrum interface contains a visual image picker whose detailed exploration is inherently visual, although its upload and export controls remain keyboard reachable. Browser-generated image previews, large result tables, and third-party advertising may vary by platform. Pop-under and social bar advertising can open an external page, tab, or window and may not provide the same keyboard or assistive-technology support as the toolkit. Some clipboard operations require browser permission and provide a manual-copy fallback when blocked.</p><h2>User control</h2><p>The toolkit itself does not autoplay audio or video or intentionally trap focus. Browser zoom and text scaling should remain available. Tool status messages use text in addition to color, and destructive actions are visually separated from primary actions. Visitors can block third-party advertising scripts in their browser if an ad format creates a barrier.</p><h2>Report a barrier</h2><p>Use the <a href="/contact/">Contact page</a> or <a href="https://github.com/QI-Tony/77toolkit/issues">GitHub issue tracker</a> with the page address, browser, operating system, assistive technology if applicable, steps, expected behavior, and observed behavior. Do not include private documents, tokens, or images in a public report.</p><p class="static-meta">Last reviewed: September 7, 2026</p></div>`,
    }),
    privacy: createStaticPage({
      ...
      body: `<header class="static-hero"><p class="static-eyebrow">Privacy policy</p><h1>Privacy, without vague promises</h1><p>This page explains what stays in your browser, what infrastructure providers can receive, and how advertising cookies may be used.</p></header><div class="static-prose"><h2>Tool inputs and files</h2><p>77 Toolkit's public developer, text, image, and web utilities are designed to process pasted values and selected files in the active browser tab. The site does not currently operate an account system or a public upload API for these tools, and it does not intentionally transmit tool input to a 77 Toolkit server. Closing or refreshing a page normally clears its working state.</p><h2>Website request data</h2><p>The site is delivered through Cloudflare. Like other hosting and security providers, Cloudflare may process request information such as IP address, browser and device information, requested URL, timestamps, security signals, and diagnostic logs to deliver and protect the service. See <a href="https://www.cloudflare.com/privacypolicy/">Cloudflare's Privacy Policy</a>.</p><h2>Third-party advertising and storage</h2><p>Indexable pages load pop-under and social bar advertising scripts from <code>profitableratecpmnetwork.com</code>. The advertising provider and advertisers may receive data such as IP address, browser and device information, page and referrer addresses, timestamps, and ad interaction data. They may also use cookies, local storage, or similar technologies for delivery, frequency control, measurement, fraud prevention, and personalization.</p><p>Advertising may open an external page, tab, or window. 77 Toolkit does not choose every advertisement or control the destination site's content and privacy practices. The toolkit's pasted text and selected files are not intentionally passed to the advertising scripts, but visitors should avoid placing secrets in URLs or sharing screens that expose private tool input.</p><h2>Your choices and contact</h2><p>You can block third-party scripts, pop-ups, cookies, or local storage through browser and content-blocking settings, or clear stored data after a visit. Where consent or opt-out controls are legally required, the advertising account and provider configuration must supply the appropriate regional controls. For privacy questions, use the contact options on the <a href="/contact/">Contact page</a> without including private tokens, documents, or images in a public report.</p><h2>External links</h2><p>Links and advertising destinations are governed by those services' own privacy practices. 77 Toolkit does not control their content or data handling.</p><h2>Children</h2><p>77 Toolkit is a general-purpose productivity site and is not directed to children under 13. Do not submit personal or sensitive information through public GitHub issues.</p><h2>Changes</h2><p>This policy may change when site features, providers, or legal obligations change. Material updates will be reflected by the date below.</p><p class="static-meta">Effective and last updated: September 7, 2026</p></div>`,
    }),
    terms: createStaticPage({
      slug: "terms",
      title: "Terms of Use",
      description: "Terms for using 77 Toolkit's free browser-based developer, text, image, and web utilities.",
      body: `<header class="static-hero"><p class="static-eyebrow">Terms of use</p><h1>Use the tools, verify the result</h1><p>77 Toolkit provides general-purpose browser utilities. By using the site, you agree to the terms below.</p></header><div class="static-prose"><h2>Permitted use</h2><p>You may use the public tools for lawful personal or commercial work. Do not use the site to violate rights, distribute harmful material, interfere with the service, bypass security controls, or create misleading or fraudulent content.</p><h2>No professional advice</h2><p>Results are informational and do not constitute legal, financial, medical, security, or other professional advice. A formatted value, decoded token, matching hash, passing contrast ratio, or cleaned image does not by itself prove that a wider system is correct, secure, compliant, or accessible.</p><h2>Your responsibility</h2><p>You are responsible for reviewing output, preserving original files, maintaining backups, and confirming that you have permission to process the material you provide. Do not rely on the site as the only copy of important data.</p><h2>Availability and warranties</h2><p>The site is provided on an “as is” and “as available” basis without guarantees of uninterrupted access, error-free output, compatibility, or fitness for a particular purpose. Browser behavior and third-party platform changes can affect results.</p><h2>Limitation of liability</h2><p>To the extent permitted by applicable law, the site operator is not liable for indirect, incidental, special, consequential, or data-loss damages arising from use of the site.</p><h2>Third-party services and advertising</h2><p>The site may link to or rely on services such as Cloudflare, GitHub, and third-party advertising providers. Pop-under and social bar advertisements can open external pages, tabs, or windows. Those services and destinations operate under their own terms and policies, and 77 Toolkit does not endorse every advertised product or destination.</p><h2>Changes and contact</h2><p>These terms may be updated as the service changes. Questions and good-faith problem reports can be submitted through the <a href="/contact/">Contact page</a>.</p><p class="static-meta">Effective and last updated: September 7, 2026</p></div>`,
    }),
...
async function validateBuildOutput() {
  ...
  for (const file of htmlFiles) {
    ...
    const legacyAdSenseCount = (html.match(/pagead2\.googlesyndication\.com\/pagead\/js\/adsbygoogle\.js/g) || []).length;
    const popunderLoaderCount = (html.match(/pl31227992\.profitableratecpmnetwork\.com\/73\/ef\/51\/73ef51cc952b9357951824c05b8a5c29\.js/g) || []).length;
    const socialBarLoaderCount = (html.match(/pl31227993\.profitableratecpmnetwork\.com\/e8\/0c\/f4\/e80cf4e5014c8492739d4b636100e786\.js/g) || []).length;
    ...
    if (!html.includes('<html lang="en">')) throw new Error(`Expected English document language in ${relativePath}.`);
    if (titleCount !== 1) throw new Error(`Expected one title in ${relativePath}, found ${titleCount}.`);
    if (descriptionCount !== 1) throw new Error(`Expected one meta description in ${relativePath}, found ${descriptionCount}.`);
    if (legacyAdSenseCount !== 0) throw new Error(`Legacy AdSense loader remains in ${relativePath}.`);
    if (isNotFoundPage && (popunderLoaderCount !== 0 || socialBarLoaderCount !== 0)) throw new Error("The 404 page must not load advertising scripts.");
    if (!isNotFoundPage && popunderLoaderCount !== 1) throw new Error(`Expected one popunder loader in ${relativePath}, found ${popunderLoaderCount}.`);
    if (!isNotFoundPage && socialBarLoaderCount !== 1) throw new Error(`Expected one social bar loader in ${relativePath}, found ${socialBarLoaderCount}.`);
...
function injectExistingToolContent(html, tool) {
  ...
  result = result.replace("</body>", `${createAdvertisingLoaders()}\n</body>`);
