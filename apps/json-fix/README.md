...
JSON Doctor 是一个纯浏览器端运行的 JSON 修复、格式化和美化工具。用户可以粘贴 JSON，或上传 `.json` / `.txt` 文件，应用会尝试修复常见 JSON 错误，并输出缩进清晰、便于复制和下载的格式化 JSON。
...
JSON Doctor 不包含登录、数据库或后端 API。输入内容、上传文件、修复过程、复制和下载操作都只发生在浏览器本地。
